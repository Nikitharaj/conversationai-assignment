...
class SetupWizard:
    ...
    def _trigger_moe_training(
        ...
        """Simulate MoE training with progress feedback."""
        try:
            progress_bar = st.progress(0)
            status_text = st.empty()
        ...
        if reconfigure or not existing_setup["rag_ready"]:
            with st.form("rag_setup_form"):
                ...
                if st.form_submit_button(button_text, type="primary"):
                    # Actually initialize the RAG system
                    with st.spinner("Setting up RAG system..."):
                        success = SetupWizard._initialize_rag_system(
                            embedding_model,
                            retrieval_method,
                            top_k,
                            reranking,
                        ...
                        if success:
                            ...
                            st.success("RAG system configuration completed!")
                            processed_files = st.session_state.get(
        ...
        if existing_setup["moe_trained"]:
            ...
            with st.form("moe_training_form"):
                ...
                if moe_submitted:
                    ...
                    # Actually trigger the MoE training
                    with st.spinner("Training MoE system..."):
                        success = SetupWizard._trigger_moe_training(
                            base_model,
                            training_data,
                            epochs,
                            batch_size,
                        ...
                        if success:
                            # Set up training trigger flags for main app
                            st.session_state.start_moe_training = True
                            st.session_state.selected_training_data = training_data
                            st.session_state.training_epochs = epochs
                            st.session_state.training_batch_size = batch_size
                            st.session_state.training_in_progress = True

                            st.success("✅ MoE training configuration completed!")
                            ...
                            st.error("MoE training setup failed!")
