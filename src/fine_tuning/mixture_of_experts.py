...
class MixtureOfExpertsFineTuner:
    ...
    def _train_expert(
        ...
        try:
            ...
            # Set up optimizer - only for parameters that require gradients
            trainable_params = [p for p in expert_model.parameters() if p.requires_grad]
            if not trainable_params:
                logger.warning(f"No trainable parameters found for expert {section}")
                return
    ...
    def _generate_with_expert(self, query: str, expert_name: str) -> str:
        ...
        try:
            ...
            # Comprehensive financial keywords filter to identify legitimate financial questions
            # This prevents the system from rejecting valid financial queries
            financial_keywords = [
            ...
            if "favorite color" in query.lower() or not any(
                keyword in query.lower() for keyword in financial_keywords
            ):
